...
def plot_results(results_by_model: dict):
    """Create a bar chart of pass rates by model."""
    ...
    plt.bar(models, pass_rates, color=['#4CAF50', '#2196F3', '#FF9800', '#9C27B0'])
    plt.ylabel('Pass Rate')
    plt.title('Model Comparison on Linear Bench')
    plt.ylim(0, 1)
    
    for i, rate in enumerate(pass_rates):
        plt.text(i, rate + 0.02, f'{rate:.0%}', ha='center')
