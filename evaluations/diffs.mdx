...
1. **WAL Capture**: All database writes are logged to the Write-Ahead Log 2. **wal2json**: Converts WAL entries to JSON format 3. **Change Journal**: Filtered and stored per environment/run 4. **Diff Computation**: Aggregated into inserts/updates/deletes
