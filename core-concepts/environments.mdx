...
<CardGroup cols={2}>
  <Card title="Template" icon="copy">
    A snapshot of a service's state (users, channels, messages, etc.) that serves as a starting point.
  </Card>
...
<Steps>
  ...
  <Step title="Capture Diff">
    After the agent runs, the system computes what changed (inserts, updates, deletes).
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Slack">
    **`slack_default` contains:**

    | Entity | Examples |
    |--------|----------|
    | Users | `U01AGENBOT9` (agent), `U02JOHNDOE1` (John), `U03JANEDOE2` (Jane) |
    | Channels | `#general`, `#random` |
    | Messages | Pre-seeded conversations |
    
    <Tip>Use `U01AGENBOT9` as your `impersonateUserId` - this is the agent's identity.</Tip>
  </Tab>
  <Tab title="Linear">
    **`linear_default` contains:**

    | Entity | Examples |
    |--------|----------|
    | Teams | Engineering (ENG), Growth (GRO) |
    | Users | Agent Bot (`2790a7ee-fde0-4537-9588-e233aa5a68d1`), John, Sarah |
    | Issues | ENG-1, ENG-2, etc. |
    | Workflow States | Backlog, In Progress, Done |
    
    <Tip>Use `2790a7ee-fde0-4537-9588-e233aa5a68d1` as your `impersonateUserId` - this is the agent's identity.</Tip>
  </Tab>
</Tabs>
